...
public class InteractionManager {
	...
	private InteractionManager()
	{
		...
		interactionMatrix = new double [dft.DFT2.ordinal()][gender.MALE.ordinal()][states.SICK.ordinal()][gender.MALE.ordinal()][states.SICK.ordinal()];
		
		for (int df=0;df<dft.DFT2.ordinal();df++) {
			for(int gen=0;gen<gender.MALE.ordinal();gen++) {
				for(int state=0;state<states.SICK.ordinal();state++) {
					for(int gen2=0;gen2<gender.MALE.ordinal();gen2++) {
						for(int state2=0;state2<states.SICK.ordinal();state2++) {
